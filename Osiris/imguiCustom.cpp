...
void ImGuiCustom::colorPicker_bars(const char* name, float color[3], bool* enable, bool* rainbow, float* rainbowSpeed, bool* percentageBased) noexcept
{
    ImGui::PushID(name);
    if (enable) {
        ImGui::Checkbox("##check", enable);
    ...
    if (ImGui::BeginPopup("##popup")) {
        ...
        if (rainbow && rainbowSpeed) {
            ...
            if (ImGui::BeginChild("##child", { 150.0f, 0.0f })) {
                ...
                ImGui::SetNextItemWidth(50.0f);
                ImGui::InputFloat("Speed", rainbowSpeed, 0.0f, 0.0f, "%.1f");
                ImGui::Checkbox("Percentage Based", percentageBased);
                ImGui::EndChild();
            }
        }
        ImGui::EndPopup();
...
void ImGuiCustom::colorPicker_bars(const char* name, Config::ColorToggle& colorConfig) noexcept
{
    colorPicker_bars(name, colorConfig.color, &colorConfig.enabled, &colorConfig.rainbow, &colorConfig.rainbowSpeed, &colorConfig.percentageBased);
}
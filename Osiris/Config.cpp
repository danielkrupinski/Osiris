...
void Config::load(size_t id) noexcept
{
    ...
    for (size_t i = 0; i < triggerbot.size(); i++) {
		...
		if (triggerbotJson.isMember("Trigger Aim")) triggerbotConfig.triggerAim = triggerbotJson["Trigger Aim"].asBool();
...
void Config::save(size_t id) const noexcept
{
    ...
    for (size_t i = 0; i < triggerbot.size(); i++) {
		...
		triggerbotJson["Trigger Aim"] = triggerbotConfig.triggerAim;

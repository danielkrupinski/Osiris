...
namespace valve_parser
{
	...
	{
	public:
		static bool StrEqu(Str str1, wchar_t* str2)
		{
			return str1.toString() == std::wstring(str2);
		}

		static bool StrEquI(Str str1, wchar_t* str2)
		{
			std::wstring str = str1.toString();
			return !wcsicmp(str.c_str(), str2);
		}
	};
	...
	public:
		...
		Object* ToObject();
		bool Parse();
		std::shared_ptr<Node> GetKeyByName(wchar_t* name);
	};
	...
	public:
		...
		std::shared_ptr<Node> BreadthFirstSearch(wchar_t* name, bool caseInsensitive = false)
		{
		...
		std::vector<std::shared_ptr<Node>> BreadthFirstSearchMultiple(wchar_t* name, bool caseInsensitive = false)
		{
		...
		//parse from file
		bool Load(const char* path, ENCODING encoding)
		{

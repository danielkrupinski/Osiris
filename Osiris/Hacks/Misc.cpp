...
static std::vector<DamageData> damageCount;

void Misc::damageList() noexcept
{
...
void Misc::addDamageToDamageList(GameEvent& event) noexcept
{
    const auto attacker = event.getInt("attacker");
    const auto dmg_health = event.getInt("dmg_health");
    const auto handle = interfaces->entityList->getEntity(interfaces->engine->getPlayerForUserID(attacker))->handle();

    static bool found;
    found = false;

    for (auto& target : damageCount) {
        if (target.playerHandle == handle) {
            target.damage += dmg_health;
            found = true;
            break;
        }
    }
    ...
    if (!found)
        damageCount.push_back(DamageData(dmg_health, handle));
    std::sort(damageCount.begin(), damageCount.end(), [](DamageData data1, DamageData data2) {
        return data1.damage > data2.damage;
        });
}

void Misc::clearDamageList() noexcept
{
    damageCount.clear();
}
...
void Misc::voteRevealer(GameEvent& event) noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, " \x0C\u2022BagCox\u2022 %c%s\x01 voted %c%s\x01", isLocal ? '\x01' : color, isLocal ? "You" : entity->getPlayerName().c_str(), color, votedYes ? "Yes" : "No");
}
...
void Misc::onVoteStart(const void* data, int size) noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, " \x0C\u2022BagCox\u2022 %c%s\x01 call vote (\x06%s\x01)", isLocal ? '\x01' : '\x06', isLocal ? "You" : entity->getPlayerName().c_str(), voteName(voteType));
}

void Misc::onVotePass() noexcept
{
    if (miscConfig.revealVotes)
        memory->clientMode->getHudChat()->printf(0, " \x0C\u2022BagCox\u2022\x01 Vote\x06 PASSED");
}

void Misc::onVoteFailed() noexcept
{
    if (miscConfig.revealVotes)
        memory->clientMode->getHudChat()->printf(0, " \x0C\u2022BagCox\u2022\x01 Vote\x07 FAILED");
}

...
namespace Misc
{
    ...
    static void autoStrafe(UserCmd* cmd) noexcept
    {
        ...
        if (config.misc.autoStrafe && (!config.misc.autoStrafeKey || GetAsyncKeyState(config.misc.autoStrafeKey)) && localPlayer->moveType() != MoveType::LADDER && !wasLastTimeOnGround && !(localPlayer->flags() & 1) && config.misc.autostrafestyle == 0) {
            if (cmd->mousedx > 1)
            {
                cmd->sidemove = 450.f;
            }
        ...
        if (config.misc.autoStrafe && (!config.misc.autoStrafeKey || GetAsyncKeyState(config.misc.autoStrafeKey)) && localPlayer->moveType() != MoveType::LADDER && !wasLastTimeOnGround && !(localPlayer->flags() & 1) && config.misc.autostrafestyle == 1) {

...
bool safeAddToOutbuf(char* outbuf, int& iOutbuf, char* pinyinValue, int iPinyinValue) {
	...
	for (m = 0; m < iOutbuf; m++) {	//统计已经转化的词数
		if (outbuf[m] == ' ') {
			iOutbufWord++;
		}
	}
	for (m = 0; m < iPinyinValue; m++) {	//统计多音字数目
		if (pinyinValue[m] == '|') {
			iPinyinValueWord++;
		}
	}
	bool flag = false;	//是否超出范围
	int iNewOutbuf = iOutbufWord * iPinyinValue + iPinyinValueWord * iOutbuf + iOutbufWord + iPinyinValueWord - 1 - iOutbufWord * iPinyinValueWord;//想想为啥
	if (iNewOutbuf < HZ2PY_OUTPUT_BUF_ARRAY_SIZE) {
		...
		char* tmp1 = outbuf;
		int n = 0, i, j, k;
		while (*tmp1 != '\0' || n == 0)	//注意，可能是最开始，故加上n==0
		{
			n += iPinyinValueWord;
			i = 0;
			while (*tmp1 != ' ' && *tmp1 != '\0')
...
int is_utf8_string(char* utf)
{
	...
	if (length > HZ2PY_UTF8_CHECK_LENGTH)	//只取前面特定长度的字符来验证即可
	{
...
void pinyin_utf8(char* inbuf, char* outbuf, bool m_blnSimaple = false, bool polyphone_support = false, bool m_blnFirstBig = false, bool m_blnAllBiG = false, bool m_LetterEnd = false, bool m_unknowSkip = true, bool m_filterPunc = true)
{
	...
	char* _tmp;
	char* _tmp2;
	char py_tmp[HZ2PY_MAX_PINYIN_SIZE] = "";	//原始拼音
	char py_tmp2[HZ2PY_MAX_PINYIN_SIZE] = "";	//处理后的拼音
	int uni;
	int iOutbuf = 0;
	char sep = '\'';			//分隔符
	...
	{
		if ((unsigned char)inbuf[i] < 0x80) {  //若是英文或数字
		...
		else if ((inbuf[i] & 0xE0) == 0xC0) {	//两个字符，表示是泛欧语系或斯拉夫语
			if (i + 1 >= inbuf_len) {	//最后一个不够了
				return;
			}
			if (!m_unknowSkip) {
		...
		else if ((inbuf[i] & 0xF0) == 0xE0) {	//三个字符，汉字、日语等亚洲字符
			if (i + 2 >= inbuf_len) {
				return;
			}

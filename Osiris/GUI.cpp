...
void GUI::renderEspWindow() noexcept
{
    if (window.esp) {
        ...
        if (ImGui::ListBoxHeader("##", { 125.0f, 300.0f })) {
            ...
            ImGui::PushFont(fonts.segoeui);
            static constexpr const char* dangerZone[]{ "Sentries", "Drones", "Cash", "Cash Dufflebag", "Pistol Case", "Light Case", "Heavy Case", "Explosive Case", "Tools Case", "Full Armor", "Armor", "Helmet", "Parachute", "Briefcase", "Tablet Upgrade" };

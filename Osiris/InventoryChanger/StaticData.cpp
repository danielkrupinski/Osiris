    ...
    void initItemData(ItemSchema* itemSchema, std::vector<int>& lootListIndices) noexcept
    {
        for (const auto& node : itemSchema->itemsSorted) {
            ...
            if (itemTypeName == "#CSGO_Type_Knife" && item->getRarity() == 6) {
                ...
                _gameItems.emplace_back(Type::Collectible, item->getRarity(), weaponID, _collectibles.size() - 1, inventoryImage);
            } else if (itemTypeName == "#CSGO_Tool_Name_TagTag") {
                _gameItems.emplace_back(Type::NameTag, item->getRarity(), weaponID, 0, inventoryImage);
            } else if (item->isPatchable()) {
                _gameItems.emplace_back(Type::Agent, item->getRarity(), weaponID, 0, inventoryImage);
                ...
                _gameItems.emplace_back(Type::Case, item->getRarity(), weaponID, _cases.size() - 1, inventoryImage);
            } else if (itemTypeName == "#CSGO_Tool_WeaponCase_KeyTag") {
                _gameItems.emplace_back(Type::CaseKey, item->getRarity(), weaponID, 0, inventoryImage);
            } else if (const auto tool = item->getEconTool()) {
                if (std::strcmp(tool->typeName, "season_pass") == 0)
                    _gameItems.emplace_back(Type::OperationPass, item->getRarity(), weaponID, 0, inventoryImage);
                else if (std::strcmp(tool->typeName, "stattrak_swap") == 0)
                    _gameItems.emplace_back(Type::StatTrakSwapTool, item->getRarity(), weaponID, 0, inventoryImage);
                else if (std::strcmp(tool->typeName, "fantoken") == 0)
                    _gameItems.emplace_back(Type::ViewerPass, item->getRarity(), weaponID, 0, inventoryImage);
            }
        }
    }

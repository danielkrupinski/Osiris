...
public:
    void addPatch(int id, StaticData::ItemName name, EconRarity rarity, std::string_view inventoryImage)
    {
        ...
        addItem(game_items::GameItem::patch(rarity, patchKits.size() - 1, inventoryImage));
    }
    ...
    void addGraffiti(int id, StaticData::ItemName name, EconRarity rarity, std::string_view inventoryImage)
    {
        ...
        addItem(game_items::GameItem::graffiti(rarity, index, inventoryImage));
        addItem(game_items::GameItem::sealedGraffiti(rarity, index, inventoryImage));
    }
    ...
    void addSticker(int id, StaticData::ItemName name, EconRarity rarity, std::string_view inventoryImage, std::uint32_t tournamentID, TournamentTeam tournamentTeam, int tournamentPlayerID, bool isGoldenSticker)
    {
        ...
        addItem(game_items::GameItem::sticker(rarity, stickerKits.size() - 1, inventoryImage));
    }

    void addMusic(int musicID, StaticData::ItemName name, std::string_view inventoryImage)
    {
        musicKits.emplace_back(musicID, name);
        addItem(game_items::GameItem::musicKit(EconRarity::Blue, musicKits.size() - 1, inventoryImage));
    }

    void addVanillaKnife(WeaponId weaponID, std::string_view inventoryImage)
    {
        addItem(game_items::GameItem::skin(EconRarity::Red, weaponID, vanillaPaintIndex, inventoryImage));
    }

    void addCollectible(EconRarity rarity, WeaponId weaponID, bool isOriginal, std::string_view inventoryImage)
    {
        addItem(game_items::GameItem::collectible(rarity, weaponID, static_cast<std::size_t>(isOriginal), inventoryImage));
    }

    void addVanillaSkin(WeaponId weaponID, std::string_view inventoryImage)
    {
        addItem(game_items::GameItem::skin(EconRarity::Default, weaponID, vanillaPaintIndex, inventoryImage));
    }

    void addServiceMedal(EconRarity rarity, std::uint32_t year, WeaponId weaponID, std::string_view inventoryImage)
    {
        addItem(game_items::GameItem::serviceMedal(rarity, weaponID, static_cast<std::size_t>(year), inventoryImage));
    }

    void addTournamentCoin(EconRarity rarity, WeaponId weaponID, std::uint32_t tournamentEventID, std::string_view iconPath)
    {
        addItem(game_items::GameItem::tournamentCoin(rarity, weaponID, static_cast<std::size_t>(tournamentEventID), iconPath));
    }
    ...
    {
        addItem(game_items::GameItem::gloves(rarity, weaponID, paintKits.size() - 1, iconPath));
    }

    void addSkinWithLastPaintKit(EconRarity rarity, WeaponId weaponID, std::string_view iconPath)
    {
        addItem(game_items::GameItem::skin(rarity, weaponID, paintKits.size() - 1, iconPath));
    }

    void addNameTag(EconRarity rarity, WeaponId weaponID, std::string_view iconPath)
    {
        addItem(game_items::GameItem::nameTag(rarity, weaponID, 0, iconPath));
    }

    void addAgent(EconRarity rarity, WeaponId weaponID, std::string_view iconPath)
    {
        addItem(game_items::GameItem::agent(rarity, weaponID, 0, iconPath));
    }

    void addCase(EconRarity rarity, WeaponId weaponID, std::size_t descriptorIndex, std::string_view iconPath)
    {
        addItem(game_items::GameItem::crate(rarity, weaponID, descriptorIndex, iconPath));
    }

    void addCaseKey(EconRarity rarity, WeaponId weaponID, std::string_view iconPath)
    {
        addItem(game_items::GameItem::caseKey(rarity, weaponID, 0, iconPath));
    }

    void addOperationPass(EconRarity rarity, WeaponId weaponID, std::string_view iconPath)
    {
        addItem(game_items::GameItem::operationPass(rarity, weaponID, 0, iconPath));
    }

    void addStatTrakSwapTool(EconRarity rarity, WeaponId weaponID, std::string_view iconPath)
    {
        addItem(game_items::GameItem::statTrakSwapTool(rarity, weaponID, 0, iconPath));
    }

    void addSouvenirToken(EconRarity rarity, WeaponId weaponID, std::uint32_t tournamentEventID, std::string_view iconPath)
    {
        addItem(game_items::GameItem::souvenirToken(rarity, weaponID, static_cast<std::size_t>(tournamentEventID), iconPath));
    }

    void addViewerPass(EconRarity rarity, WeaponId weaponID, std::uint32_t tournamentEventID, std::string_view iconPath)
    {
        addItem(game_items::GameItem::viewerPass(rarity, weaponID, static_cast<std::size_t>(tournamentEventID), iconPath));
    }

    const auto& getStickerKit(const game_items::GameItem& item) const
    {
        assert(item.isSticker());
        return stickerKits[item.dataIndex];
    }

    const auto& getMusicKit(const game_items::GameItem& item) const
    {
        assert(item.isMusic());
        return musicKits[item.dataIndex];
    }

    const auto& getPaintKit(const game_items::GameItem& item) const
    {
        assert(item.isSkin() || item.isGloves());
        return paintKits[item.dataIndex];
    }

    const auto& getGraffitiKit(const game_items::GameItem& item) const
    {
        assert(item.isGraffiti() || item.isSealedGraffiti());
        return graffitiKits[item.dataIndex];
    }

    const auto& getPatchKit(const game_items::GameItem& item) const
    {
        assert(item.isPatch());
...
private:
    void addItem(const game_items::GameItem& item)
    {
    ...
    std::vector<game_items::GameItem> gameItems;
};

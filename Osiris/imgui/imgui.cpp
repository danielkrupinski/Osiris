...
// Known size data hash
// It is ok to call ImHashData on a string with known length but the ### operator won't be supported.
// We use a non-orthodox variant of fnv1a (xoring with OFFSET_BASIS on both ends) to allow caller to pass a zero seed.
ImU32 ImHashData(const void* data_p, size_t data_size, ImU32 seed)
{
    const unsigned int FNV_PRIME = 0x01000193;    // 16777619u;
    const unsigned int OFFSET_BASIS = 0x811C9DC5; // 2166136261u;
    seed ^= OFFSET_BASIS;
    ImU32 hash = seed;
    const unsigned char* data = (const unsigned char*)data_p;
    while (data_size-- != 0)
        hash = (hash ^ *data++) * FNV_PRIME;
    return hash ^ OFFSET_BASIS;
}

...
[[nodiscard]] EconItemFunctions createEconItemFunctions(const PatternFinder& clientPatternFinder)
{
    return EconItemFunctions{
        ...
        .setCustomName = clientPatternFinder("E8 ? ? ? ? 8B 46 78 C1 E8 0A A8 01 74 13 8B 46 34"_pat).add(1).abs().as<csgo::SetCustomName>()
#elif IS_LINUX()
        ...
        .removeDynamicAttribute = clientPatternFinder("E8 ? ? ? ? 80 3D ? ? ? ? ? 75 14 48 8D 3D ? ? ? ? E8 ? ? ? ? 85 C0 0F 85 ? ? ? ? C7 45"_pat).add(1).abs().as<csgo::RemoveDynamicAttribute>(),
        .setCustomName = clientPatternFinder("E8 ? ? ? ? 8B 83 ? ? ? ? E9 ? ? ? ? 48 8B 05 ? ? ? ? 44 8B A0"_pat).add(1).abs().as<csgo::SetCustomName>()
#endif
    };
}

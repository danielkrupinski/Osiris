...
struct ClientPatterns {
    [[nodiscard]] static consteval auto addClientPatterns(auto clientPatterns) noexcept
    {
        return clientPatterns
            ...
            .template addPattern<WorldToProjectionMatrixPointer, CodePattern{"E6 4C 8D 05 ? ? ? ? 48 8D 0D"}.add(4).abs()>()
            .template addPattern<ViewToProjectionMatrixPointer, CodePattern{"0D ? ? ? ? 48 8B 38 48 8D 15"}.add(1).abs()>()

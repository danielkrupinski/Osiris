...
struct PanoramaUiPanelPatterns {
    [[nodiscard]] static consteval auto addClientPatterns(auto clientPatterns) noexcept
    {
        return clientPatterns
            ...
            .template addPattern<SetVisibleFunctionOffset, CodePattern{"78 ? 48 8B 07 FF 90 ? ? ? ? 4C 89 F7 E8 ? ? ? ? 31"}.add(7).read()>()
            .template addPattern<GetAttributeStringFunctionOffset, CodePattern{"FF 90 ? ? ? ? 80 BB 90 ? ? ? ? 48 89 C2"}.add(2).read()>()
            .template addPattern<SetAttributeStringFunctionOffset, CodePattern{"48 89 DA 0F B7 30 48 8B 07 FF 90 ? ? ? ? B8 01 00 00 00"}.add(11).read()>();
    }
    ...
    [[nodiscard]] static consteval auto addPanoramaPatterns(auto panoramaPatterns) noexcept
    {
        return panoramaPatterns
            ...
            .template addPattern<OffsetToPanelId, CodePattern{"? 00 0F 84 ? ? ? ? ? 8D ? ? 48 83 C4"}.read()>()

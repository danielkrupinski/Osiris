...
struct PlantedC4Patterns {
    ...
    {
        return patternFinders.clientPatternFinder("74 ? 44 38 AE ? ? ? ? 74 ? 49"_pat).add(5).readOffset<BombTickingOffset>();
    }

    [[nodiscard]] BombBlowTimeOffset m_flC4Blow() const noexcept
    {
        return patternFinders.clientPatternFinder("F3 0F 10 ? ? ? ? ? BB FF"_pat).add(4).readOffset<BombBlowTimeOffset>();
    }

    [[nodiscard]] BombDefuserOffset m_hBombDefuser() const noexcept
    {
        return patternFinders.clientPatternFinder("E8 ? ? ? ? 8B ? ? ? ? ? 45 33 ? 4C 8B ? ? ? ? ? 83"_pat).add(7).readOffset<BombDefuserOffset>();
    }

    [[nodiscard]] BombDefuseEndTimeOffset m_flDefuseCountDown() const noexcept
    {
        return patternFinders.clientPatternFinder("74 11 F3 0F 10 B7 ? ? ? ? 48"_pat).add(6).readOffset<BombDefuseEndTimeOffset>();
    }
};

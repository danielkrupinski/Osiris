...
struct ClientPatterns {
    [[nodiscard]] static consteval auto addClientPatterns(auto clientPatterns) noexcept
    {
        return clientPatterns
            .template addPattern<MainMenuPanelPointer, CodePattern{"48 83 EC ? 48 8B 05 ? ? ? ? 48 8D 15 ? ? ? ? 48 8B ? ? 48 8B ?"}.add(7).abs()>()
            ...
            .template addPattern<SetSceneObjectAttributeFloat4, CodePattern{"0F 11 74 24 ? E8 ? ? ? ? FF C6 48 ? C3"}.add(6).abs()>()

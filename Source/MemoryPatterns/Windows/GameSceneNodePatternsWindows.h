...
struct GameSceneNodePatterns {
    [[nodiscard]] static consteval auto addClientPatterns(auto clientPatterns) noexcept
    {
        return clientPatterns
            ...
            .template addPattern<OffsetToGameSceneNodeOwner, CodePattern{"44 39 ? ? ? ? ? 75 ? 48 8B ? ? EB"}.add(12).read()>()
            .template addPattern<OffsetToChildGameSceneNode, CodePattern{"85 F6 74 ? ? 8B ? ? 48 85 DB"}.add(7).read()>()

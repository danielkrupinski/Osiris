...
Memory::Memory(const PatternFinder& clientPatternFinder, const PatternFinder& enginePatternFinder, csgo::ClientPOD* clientInterface, const RetSpoofGadgets& retSpoofGadgets) noexcept
    ...
    : lineGoesThroughSmoke{ retSpoofGadgets.client, clientPatternFinder("E8 ? ? ? ? 8B 4C 24 30 33 D2"_pat).add(1).abs().get() },
      ...
      findOrCreateEconItemViewForItemID{ retSpoofGadgets.client, clientPatternFinder("E8 ? ? ? ? 8B CE 83 C4 08"_pat).add(1).abs().get() },
      makePanoramaSymbolFn{ retSpoofGadgets.client, clientPatternFinder("E8 ? ? ? ? 0F B7 45 0E 8D 4D 0E"_pat).add(1).abs().get() }
#elif IS_LINUX()
    : lineGoesThroughSmoke{ retSpoofGadgets.client, clientPatternFinder("E8 ? ? ? ? 44 0F B6 4D ? 84 C0"_pat).add(1).abs().get() },
      weaponSystem{ retSpoofGadgets.client, clientPatternFinder("48 8B 3D ? ? ? ? 48 8B 4D F8"_pat).add(3).abs().deref().get() },
      inventoryManager{ csgo::InventoryManager::from(retSpoofGadgets.client, clientPatternFinder("48 8D 3D ? ? ? ? E8 ? ? ? ? 48 8D 15 ? ? ? ? C9"_pat).add(3).abs().as<csgo::InventoryManagerPOD*>()) },
      findOrCreateEconItemViewForItemID{ retSpoofGadgets.client, clientPatternFinder("E8 ? ? ? ? 49 89 C5 4D 85 E4"_pat).add(1).abs().get() },
      makePanoramaSymbolFn{ retSpoofGadgets.client, clientPatternFinder("E8 ? ? ? ? 0F B7 45 A0 31 F6"_pat).add(1).abs().get() }
#endif
{
#if IS_WIN32() || IS_WIN64()
    ...
    setAbsOrigin = clientPatternFinder("E8 ? ? ? ? EB 19 8B 07"_pat).add(1).abs().as<decltype(setAbsOrigin)>();
    ...
    drawScreenEffectMaterial = clientPatternFinder("E8 ? ? ? ? 83 C4 0C 8D 4D F8"_pat).add(1).abs().get();
    ...
    getDecoratedPlayerName = clientPatternFinder("E8 ? ? ? ? 66 83 3E"_pat).add(1).abs().as<decltype(getDecoratedPlayerName)>();
    ...
    createBaseTypeCache = clientPatternFinder("E8 ? ? ? ? 8D 4D 0F"_pat).add(1).abs().as<csgo::CreateBaseTypeCache>();
    ...
    keyValuesAllocClient = ReturnAddress{ clientPatternFinder("E8 ? ? ? ? 83 C4 08 84 C0 75 10"_pat).add(1).abs().add(0x3E).get() };

    shouldDrawFogReturnAddress = ReturnAddress{ clientPatternFinder("E8 ? ? ? ? 8B 0D ? ? ? ? 0F B6 D0"_pat).add(1).abs().add(82).get() };
#elif IS_LINUX()
    ...
    playerResource = clientPatternFinder("48 8B 15 ? ? ? ? 48 63 C3"_pat).add(3).abs().as<csgo::PlayerResource**>();
    ...
    viewRender = clientPatternFinder("0F 85 ? ? ? ? 48 8D 05 ? ? ? ? 45 89 F8"_pat).add(9).abs().deref().as<csgo::ViewRender*>();
    clearHudWeapon = clientPatternFinder("E8 ? ? ? ? C6 45 AE 01"_pat).add(1).abs().as<decltype(clearHudWeapon)>();
    ...
    setAbsOrigin = clientPatternFinder("E8 ? ? ? ? 4D 63 B5"_pat).add(1).abs().as<decltype(setAbsOrigin)>();
    plantedC4s = clientPatternFinder("48 8D 3D ? ? ? ? 42 C6 44 28"_pat).add(3).abs().as<decltype(plantedC4s)>();
    gameRules = clientPatternFinder("48 8D 1D ? ? ? ? 48 8B 3B 48 85 FF 74 06"_pat).add(3).abs().as<csgo::EntityPOD**>();
    predictionRandomSeed = clientPatternFinder("41 8D 56 FF 31 C9"_pat).add(-14).abs().as<int*>();
    registeredPanoramaEvents = clientPatternFinder("E8 ? ? ? ? 44 0F B7 9D"_pat).add(1).abs().add(12).abs().as<decltype(registeredPanoramaEvents)>();
    moveData = clientPatternFinder("48 8D 0D ? ? ? ? 48 89 DE 4C 89 FF"_pat).add(3).abs().deref().as<csgo::MoveData*>();
    moveHelperPtr = clientPatternFinder("48 8D 05 ? ? ? ? 48 8B 38 48 8B 07 FF 50 58 41 0F B7 F7"_pat).add(3).abs().deref().as<csgo::MoveHelperPOD*>();

    panoramaMarshallHelper = clientPatternFinder("48 89 05 ? ? ? ? 48 C7 05 ? ? ? ? ? ? ? ? 48 8D 15"_pat).add(3).abs().as<decltype(panoramaMarshallHelper)>();
    createBaseTypeCache = clientPatternFinder("E8 ? ? ? ? 48 89 DE 5B 48 8B 10"_pat).add(1).abs().as<csgo::CreateBaseTypeCache>();

    localPlayer.init(clientPatternFinder("83 FF FF 48 8B 05"_pat).add(6).abs().as<csgo::EntityPOD**>());
#endif
}

...
class Chams {
public:
    bool render(Backtrack& backtrack, const Engine& engine, const ClientInterfaces& clientInterfaces, const OtherInterfaces& interfaces, const Memory& memory, Config& config, void*, void*, const ModelRenderInfo&, matrix3x4*) noexcept;
    static void updateInput(Config& config) noexcept;
private:
    void renderPlayer(Backtrack& backtrack, const Engine& engine, const StudioRender& studioRender, const Memory& memory, Config& config, const Entity& player) noexcept;

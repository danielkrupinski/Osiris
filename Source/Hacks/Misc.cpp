...
void Misc::fastPlant(EngineTrace& engineTrace, const Interfaces& interfaces, UserCmd* cmd) noexcept
{
    ...
    if (const auto activeWeapon = Entity::from(retSpoofGadgets.client, localPlayer.get().getActiveWeapon()); activeWeapon.getThis() == 0 || activeWeapon.getNetworkable().getClientClass()->classId != ClassId::C4)
        return;
...
void Misc::quickReload(const ClientInterfaces& clientInterfaces, const Interfaces& interfaces, UserCmd* cmd) noexcept
{
    if (miscConfig.quickReload) {
        ...
        if (const auto activeWeapon = Entity::from(retSpoofGadgets.client, localPlayer.get().getActiveWeapon()); activeWeapon.getThis() != 0 && activeWeapon.isInReload() && activeWeapon.clip() == activeWeapon.getWeaponData()->maxClip) {
...
void Misc::purchaseList(const Engine& engine, const ClientInterfaces& clientInterfaces, const Interfaces& interfaces, const Memory& memory, const GameEvent* event) noexcept
{
    ...
    if (event) {
        ...
        case fnv::hash("item_purchase"): {
            if (const auto player = Entity::from(retSpoofGadgets.client, clientInterfaces.getEntityList().getEntity(engine.getPlayerForUserID(event->getInt("userid")))); player.getThis() != 0 && localPlayer && localPlayer.get().isOtherEnemy(memory, player)) {
